...
module.exports = class ServersComponent {
  ...
  _enableHttpWebSocket(socket) {
    ...
    if (!socket ||
      ...
      let e = 'Please see the documentation in order to use http webSocket';
      throw new Error(e);
    }
  ...
  _createHttpsServer(config, cluster, routes) {
    ...
    if (!routes || !Object.keys(routes.app).length) {
      let e = 'Please see the documentation in order to use https server';
      throw new Error(e);
    }
    this._onServerCreated();
  }
  ...
  _createUdpServer(config, cluster, socket) {
    ...
    if (!socket ||
      ...
      let e = 'Please see the documentation in order to use udp server';
      throw new Error(e);
    }
    let udpSettings = {
      port: 3335,
  ...
  _createTcpServer(config, cluster, socket) {
    ...
    if (!socket ||
      ...
      let e = 'Please see the documentation in order to use tcp server';
      throw new Error(e);
    }
    let tcpSettings = {
      port: 3334,
